...
**同步策略（双向同步，本地库为主，8/13 落地）**：

- 推送（原有）：评估完成 / 教师保存后，后端调用批量接口写回多维表格。
- 拉取（新增）：`POST /api/feishu/bitable/pull`（手动触发，前端「从表格导入」按钮）。把教师在多维表格里拉动的**教师列**写回本地库：responses 表的教师评分 / 教师标签 / 教师批注 / 状态，students 表的评语草稿。
- 字段所有权切分：后端只写内容与 AI 列（原始/清洗文本、AI 评分摘要、置信度、建议标签、加分项、来源、更新时间），教师只写上述教师列；拉取只读教师列，双写字段天然错开，绝大多数情况无冲突。
- 防回环：`feishu_bindings` 新增 `last_synced_hash`，推送后对教师列做规范化哈希快照；拉取时远端哈希与快照一致即视为"自己推上去的回声"，跳过；不一致才回写。
- 安全方向：远端「状态=教师已审」只会把本地推向已审，**绝不撤销**已有批改；远端评分摘要按"立意:A；选材:B+"格式解析回维度字典（未知维度名原样保留）。
- 未匹配远端行（教师手加、本地无绑定）只计数报告（`unmatched_remote`），不自动创建本地记录；远端删除也不联动删本地。
...
> **实现状态（2026-08-10 晚，对本节设计的修订）**：回调通道没有走 HTTP 回调（无需公网回调地址、无需 challenge 验证），改用**飞书长连接**——新增 `feishu/ws_listener.py` 常驻进程（在 backend 根目录执行 `python -m feishu.ws_listener`），基于 lark-oapi `ws.Client` 接收 `card.action.trigger`（新版卡片平台的卡片回调经事件通道投递）与 `im.message.receive_v1`。HTTP `/api/feishu/card` 路由保留为备用通道，与 ws 共用分发入口 `feishu/card_actions.py`。控制台"订阅方式"须选"使用长连接接收事件"并发布新版本。启动姿势、验证方法与踩坑记录见《多维表格联调记录_2026-08-10.md》Module C 章节。
